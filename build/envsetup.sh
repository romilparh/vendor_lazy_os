...
function breakfast()
{
    target=$1
    local variant=$2
    LAZY_DEVICES_ONLY="true"
    unset LUNCH_MENU_CHOICES
    add_lunch_combo full-eng
    for f in `/bin/ls vendor/lazy/vendorsetup.sh 2> /dev/null`
        do
            echo "including $f"
            . $f
        done
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the Lazy model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch lazy_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/lazy-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        if (adb shell getprop ro.lazy.device | grep -q "$LAZY_BUILD"); then
...
EOF
            ...
            echo "The connected device does not appear to be $LAZY_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function installboot()
{
    ...
    if (adb shell getprop ro.lazy.device | grep -q "$LAZY_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.lazy.device | grep -q "$LAZY_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function repopick() {
    T=$(gettop)
    $T/vendor/lazy/build/tools/repopick.py $@
}

function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    if [ ! -z $LAZY_FIXUP_COMMON_OUT ]; then
...
if [ -d $(gettop)/prebuilts/snapdragon-llvm/toolchains ]; then
    case `uname -s` in
        ...
        *)
            ...
            export SDCLANG_LTO_DEFS=$(gettop)/vendor/lazy/build/core/sdllvm-lto-defs.mk
            ;;
    esac
fi
